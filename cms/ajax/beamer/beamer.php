...
function get_items($type, $option = DD_FLAG, $section_id = 0, $item_id = 0)
{
	if($type === 'tours')
	{
		$result_items = get_tours($option, $section_id, $item_id);
	}
	elseif ($type === 'blogs') 
	{
...
function get_tours($option, $section_id, $item_id)
{
	$result = array();

	if($option === DD_FLAG)
	{
		$result = fetch_all("SELECT t.`id` AS ind, pmd.`heading` AS label
			FROM `tours` t 
	...
	elseif($option === CONTENT_FLAG)
	{
		if($section_id != 0)
		{
            ...
            $main_sql = "SELECT t.`id` AS ind, 
                    IF(t.`from_price`, REPLACE(FORMAT(t.`from_price`, 2), '.00', ''), 'POA') AS rate, 
                    pmd.`heading` AS label, 
                    pmd.`menu_label` AS menuLabel, pmd.`short_description` AS details, pmd.`full_url` AS uri, pmd.`url` AS url,
                    pmd.`thumb_photo` AS thumbPhoto, pmd.`title`, 
                    bcsi.`id` AS itemInd, bcsi.`rank` AS itemRank,'tours' AS type
                    FROM `beamer_campaign_section_items` bcsi 
                    LEFT JOIN `tours` t
                    ON(t.`id` = bcsi.`item_id`)
                    LEFT JOIN `page_meta_data` pmd
                    ON(pmd.`id` = t.`page_meta_data_id`)
                    WHERE pmd.`status` = 'A'
