...
function do_login()
{
    ...
    if($email && $password && strlen($password) > 5 && $captcha_is_valid && !$user_is_locked)
    {
        ...
        if(!empty($user_details))
        {
            ...
            setcookie("siteUserId",         SHA1($user_details['user_id']), $cookieExpire);
            
            $s_accessid = $user_details['access_id'];
            ...
            if(!isset($_COOKIE['lastUserLoggedin']))
            {
                setcookie("lastLoggedinUserId",   $last_user_loggedin['user_id'], $cookieExpire);
                setcookie("lastLoggedinName",     $last_user_loggedin['name'], $cookieExpire);
                setcookie("lastLoggedinDate",     $last_user_loggedin['last_login_date'], $cookieExpire);
            }
...
{
    $valid = 1;
    if (!isset($_COOKIE['siteUserId']) && $do != "logout")
    {
