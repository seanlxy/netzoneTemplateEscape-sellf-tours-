	...
	if( $id != 1 )
	{
	    ...
	    if( $full_url )
	    {
	        ...
	        $sql = "SELECT gp.`id`
                ...
                WHERE imp.`imppage_id` = 5 
                LIMIT 1";
          
            $tourPageId = fetch_value($sql);
           
            if (!empty($tourPageId) && $id == $tourPageId) {
                
                run_query("	UPDATE `page_meta_data` pmd
                    		INNER JOIN `tours` t
                    	   	ON(pmd.`id` = t.`page_meta_data_id`)
                    		SET pmd.`full_url` = CONCAT('/{$full_url}', '/', pmd.`url`)
                    		WHERE t.`id` != ''");
            }
	    }
    }
