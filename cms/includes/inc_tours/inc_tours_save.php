...
function save_item()
{
    ...
    if($photo)
    {
        ...
        if($photo != $current_photo)
        {
            ...
            if( is_file($photo_full) )
            {
                ...
                $resizer_class->resizer($upload_dir_full_path, $photo_full, 600, 450, $thumb_name);
    ...
    $tours_data                 = array();
    $tours_data['from_price']   = sanitize_input('from_price');
    $tours_data['gallery_id']   = sanitize_input('gallery_id', FILTER_SANITIZE_NUMBER_INT);
    $tours_data['slideshow_id'] = sanitize_input('slideshow_id', FILTER_SANITIZE_NUMBER_INT);

    $tours_data['beds']         = sanitize_input('beds');
    $tours_data['pax']          = sanitize_input('pax');
    $tours_data['sqm']          = sanitize_input('sqm');
    $tours_data['longitude']    = $_POST['longitude'];
    $tours_data['latitude']     = $_POST['latitude'];
    $tours_data['google_map_address']= $_POST['google_map_address'];
    $latitude                   = $tours_data['latitude'];
    $longitude                  = $tours_data['longitude'];
    $google_map_address         = $tours_data['google_map_address'];
    $tours_data['address']      = $_POST['address'];


    update_row($tours_data,'tours', "WHERE id = '{$id}'");

    // save features
    run_query("DELETE FROM `tours_has_feature` WHERE `tours_id` = '{$id}'");
    ...
    if(count($feature_id) > 0)
    {
        for($i=0;$i<count($feature_id);$i++)
        {
            ...
            insert_row($ins_arr, 'tours_has_feature');
    ...
    run_query("DELETE FROM `tour_has_compendium_section` WHERE `tour_id` = '$id'");
    ...
    if(!empty($generic_sections))
    {
        foreach ($generic_sections as $gen) {
            ...
            $insert_gen_section['tour_id']           = $id;

            insert_row($insert_gen_section, 'tour_has_compendium_section');
        }
    }
    ...
    if(count($compendium_content) > 0)
    {
        for ($z=0; $z < count($compendium_content); $z++)
        {
            ...
            if($is_map[$z] == 1)
            {
                ...
                $insert_comp_section['tour_id']           = $id;
                
                insert_row($insert_comp_section, 'tour_has_compendium_section');
            }
            else
            {
