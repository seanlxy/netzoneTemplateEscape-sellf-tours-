...
if( !empty($tabs) && count($tabs) > 1 )
{
	...
	$gallery_tabs .= '<a class="shuffle-trigger btn btn-gallery active" title="All Photos" href="#" data-group="all">All Photos</a>';

	foreach ($tabs as $tab)
	{
		$gallery_ids[$tab['id']] = $tab['menu_label'];
		$gallery_tabs .= '<a class="shuffle-trigger btn btn-gallery" title="'.$tab['menu_label'].'" href="#" data-group="'.$tab['hashed_key'].'">'.$tab['menu_label'].'</a>';
	}
